...
if (!file.exists(state_path)) {
  we_stop_check("STOP — DOMAIN MODEL checkS NOT COMPLETE", "BOOTSTRAP_ENTRY_check")
}
state <- readRDS(state_path)
if (!isTRUE(state$stage1_complete)) {
  we_stop_check("STOP — DOMAIN MODEL checkS NOT PASSED", "BOOTSTRAP_ENTRY_check")
}
...
not_run_summary <- function(analysis, full_estimate, reason) {
  data.frame(
    analysis = analysis, full_data_contrast = full_estimate,
    bootstrap_mean = NA_real_, bootstrap_median = NA_real_, bootstrap_SD = NA_real_,
    percentile_2_5 = NA_real_, percentile_97_5 = NA_real_,
    negative_replicate_fraction = NA_real_, positive_replicate_fraction = NA_real_,
    n_success = 0L, n_attempt = 0L, n_fail = 0L, failure_fraction = NA_real_,
    seed = WE_SEED, rho_used = WE_RHO_FIXED, interval_method = "NOT_RUN",
...
run_bootstrap_analysis <- function(analysis) {
  ...
  if (is.null(fit_state)) {
    we_stop_check("STOP — DOMAIN MODEL checkS NOT COMPLETE",
                   paste0(analysis, "_BOOTSTRAP_ENTRY_check"))
  }
  if (!isTRUE(fit_state$support_pass)) {
    reason <- "DOMAIN_SENSITIVITY_FIXED_CONTRAST_UNSUPPORTED"
    we_log(analysis, "bootstrap not run:", reason)
    return(not_run_summary(analysis, fit_state$contrast, reason))
  }

  d_full <- we_prepare_domain(master, analysis)
  required <- unique(c(WE_REQUIRED_FIELDS, "AR.start"))
  ...
  draw_path <- we_path("05_BOOTSTRAP", paste0(prefix, "_BOOTSTRAP_SITE_DRAW_SEQUENCE.csv"))
  ...
  checkpoint_path <- we_path("05_BOOTSTRAP", paste0(prefix, "_BOOTSTRAP_CHECKPOINT.rds"))
  ...
  flush_bootstrap <- function() {
    ...
    we_write_csv(attempt_log,
                   we_path("05_BOOTSTRAP", paste0(prefix, "_BOOTSTRAP_ATTEMPT_LOG.csv")))
    we_write_csv(success_output,
                   we_path("05_BOOTSTRAP", paste0(prefix, "_CONTRAST_BOOTSTRAP.csv")))
    we_write_csv(failure_rows,
                   we_path("05_BOOTSTRAP", paste0(prefix, "_BOOTSTRAP_FAILURE_LOG.csv")))
    ...
    progress <- data.frame(
      ...
      fixed_rho = WE_RHO_FIXED, formula_changed = FALSE, k_changed = FALSE,
      family_changed = FALSE, stringsAsFactors = FALSE
    )
    we_write_csv(progress,
                   we_path("05_BOOTSTRAP", paste0(prefix, "_BOOTSTRAP_PROGRESS.csv")))
  ...
  for (attempt in seq_len(WE_BOOT_INITIAL_BATCH)) {
    ...
    result <- we_run_domain_boot_attempt(
      attempt, analysis_data, draw_sequence, WE_RHO_FIXED
    )
    ...
    if (!result$success && result$failure_category == "STRUCTURAL_ERROR") {
      we_stop_check("STOP — DOMAIN BOOTSTRAP INITIAL_BATCH FAILURE",
  ...
  we_write_csv(
    ...
    we_path("05_BOOTSTRAP", paste0(prefix, "_BOOTSTRAP_COMPUTATIONAL_INITIAL_BATCH.csv"))
  )
  if (!structural_ok || initial_batch_rate > WE_BOOT_MAX_FAILURE_RATE) {
    we_stop_check(
      "STOP — DOMAIN BOOTSTRAP INITIAL_BATCH FAILURE", paste0(analysis, "_BOOTSTRAP_INITIAL_BATCH"),
  ...
  if (sum(attempt_log$success) < WE_BOOT_TARGET_SUCCESS) {
    ...
    parallel::clusterExport(
      cluster, c("boot_data_global", "boot_draw_global"), envir = environment()
    )
    ...
    repeat {
      ...
      batch <- parallel::parLapply(cluster, attempts, function(i) {
        we_run_domain_boot_attempt(
          i, boot_data_global, boot_draw_global, WE_RHO_FIXED
        )
      })
  ...
  if (n_success < WE_BOOT_TARGET_SUCCESS || n_attempt > WE_BOOT_DRAWS ||
    ...
    we_stop_check(
      "STOP — DOMAIN BOOTSTRAP FAILURE RATE TOO HIGH",
  ...
  we_write_csv(summary,
                 we_path("05_BOOTSTRAP", paste0(prefix, "_CONTRAST_SUMMARY.csv")))
  we_write_csv(
    data.frame(
    ...
    we_path("05_BOOTSTRAP", paste0(prefix, "_BOOTSTRAP_check_SUMMARY.csv"))
  )

  png(
    we_path("05_BOOTSTRAP", paste0(prefix, "_BOOTSTRAP_CONTRAST_DISTRIBUTION.png")),
...
for (analysis in c("FULL_YEAR", "APR_SEP")) {
...
we_write_csv(all_boot, we_path("05_BOOTSTRAP", "WE_DOMAIN_BOOTSTRAP_SUMMARY.csv"))
state$bootstrap_complete <- TRUE
state$bootstrap_completed_at <- format(Sys.time(), "%Y-%m-%dT%H:%M:%S%z")
state$bootstrap_summaries <- bootstrap_summaries
saveRDS(state, state_path)
we_log("All specified domain sensitivity bootstraps complete")
